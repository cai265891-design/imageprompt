...
export class CozeAPI {
   ...
   */
  async generateImagePrompt(file: File, model: string): Promise<string> {
    try {
...
原始错误：${error.message}`);
   ...
   * Simplified version without language parameter
   */
  async runWorkflowWithFileId(fileId: string, model: string): Promise<string> {
  ...
  private async tryWorkflowWithParams(
    parameters: any,
    model: string,
  ): Promise<string> {
    const requestParams = {
    ...
    if (errorEvent && errorEvent.data) {
      const errorData =
        typeof errorEvent.data === "string"
          ...
          : errorEvent.data;
      throw new Error(
        `Workflow execution failed: ${errorData.error_message || "Unknown error"}`,
      );
    }
    ...
    if (lastDataEvent) {
      ...
      if (typeof responseData === "string") {
        try {
          ...
          console.log(
            "Result data is plain string, returning as-is:",
            responseData,
          );
          return responseData;
        }
